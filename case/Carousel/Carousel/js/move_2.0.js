...
function move(dom,json,callback){
	...
	for (let attr in json) {//json{width:1130,height:350}
		//console.log(json[attr]);//1130 350
		//console.log(attr);//width height
		dom.timer[attr] = setInterval(function(){
			//console.log(dom.timer,attr);//1 2 1 2 ...
			if (attr == "opacity") {
				var iNow = getStyle(dom,attr) * 100;
			}else{
				var iNow = parseInt(getStyle(dom,attr));
			}//iNow => start position
			var speed = (json[attr] - iNow) / 10;//json[attr] => target
			speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed);
			//console.log(iNow,json[attr]);
			//console.log(dom.timer[attr])
			if (iNow == json[attr]) {
				clearInterval(dom.timer[attr]);
				var count = 0;
				delete dom.timer[attr];//clear timers
				//console.log(dom.timer,dom.timer[attr]);
				//console.log(delete dom.timer[attr])
